document.addEventListener('DOMContentLoaded', function() {
    ...
    // This block handles pre-populating fields if there were server-side errors
    // The initial values are passed from Flask via Jinja2's tojson filter
    // and are expected to be available in the global scope when this script runs.
    if (typeof initialSubmissionType !== 'undefined') {
        if (submissionType) submissionType.value = initialSubmissionType;
    }
    if (typeof initialProjectTitle !== 'undefined') {
        if (projectTitle) projectTitle.value = initialProjectTitle;
    }
    if (typeof initialReflectionStory !== 'undefined') {
        if (reflectionStory) reflectionStory.value = initialReflectionStory;
    }
    if (typeof initialConfirmationCheck !== 'undefined') {
        if (confirmationCheck) confirmationCheck.checked = initialConfirmationCheck === 'true';
    }
    ...
    function validateStep2() {
        ...
        if (reflectionStory && (!reflectionStory.value.trim() || reflectionStory.value.trim().length < 50)) {
            showError('reflection-story-client-error', 'Please provide a detailed reflection (at least 50 characters)');
            isValid = false;
        } else {
    ...
    function updateReviewSection() {
        ...
        const reviewFilesElement = document.getElementById('review-files');
        if (reviewFilesElement) {
            if (uploadedFiles.length > 0) {
                reviewFilesElement.textContent = uploadedFiles.map(f => f.name).join(', ');
            } else {
                reviewFilesElement.textContent = 'No files selected';
            }
        }
    }
